...
1. Capability Gaps backlog + Tier 1 introspection (2026-04-17):
   ...
   - Tier 1 implemented in one batch:
     - **T1-1 pin default values**: `get_blueprint_graph_info` pin entries now include `pin_subtype`, `pin_subtype_object`, `default_value`, `default_object_path`, `default_text`, and `autogenerated_default`. Pins are emitted when they have connections, are exec pins, OR carry an explicit default — previously unconnected pins with defaults (e.g. `MakeStruct` field values, `Send Gameplay Event` tag) were silently dropped.
     - **T1-4 interface override graphs**: `get_blueprint_graph_info` now also searches `Blueprint->ImplementedInterfaces[].Graphs`, so interface overrides (e.g. `BP_PlayerCharacter_ASC_OnPlayerState`'s 11 `ITBIA_Interactable` graphs) are visible. Response adds `interface_graphs` map `{interface_name: [graph_names]}` for per-interface discovery while keeping `available_graphs` as a flat list.
     - **T1-2 + T1-3 `get_blueprint_info`** (new command): returns `parent_class` (path + short), `generated_class`, `blueprint_type`, `implemented_interfaces` (with `graph_count`), and `components` (with `is_native` flag distinguishing SCS-added vs inherited-from-parent-CDO, plus `parent_component` for SCS attach tree). Answers the basic question "what is this BP?" that previously required `strings` on the uasset binary.
   - C++ handler: `UnrealMCPBlueprintCommands::HandleGetBlueprintInfo` + dispatcher entry in `UnrealMCPBridge::ExecuteCommand`.
   - Python tool: `blueprint_tools.py` gains `get_blueprint_info`; docstring of `get_blueprint_graph_info` updated to reflect new pin default fields + `interface_graphs`.
   - Docs: `Docs/commands.md` updated with new command entry and expanded `get_blueprint_graph_info` field list.
